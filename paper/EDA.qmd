...
#| warning: false
#| message: false
# Boxplot to visualize the distribution of `game_after`
ggplot(clean_data, aes(x = drug, y = game_after, fill = dosage_level)) +
  geom_boxplot() +
  ggtitle("Boxplot of Game After Scores by Drug and Dosage Level")
...
anova_model <- aov(game_after ~ drug + dosage_level, data = clean_data)
summary(anova_model)
...
# p-value is greater than 0.05 we fail to reject Ho implies Normality
...
model_cards_dose <- aov(cards_after ~ drug + dosage_level, data = clean_data)
shapiro.test(residuals(model_cards_dose))
ggqqplot(residuals(model_cards_dose))
```
