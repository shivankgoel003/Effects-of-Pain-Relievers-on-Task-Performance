...
We did a post-hoc power analysis using @WebPower. Based on the overall $\eta^2$ from the ANOVA model, we calculated a Cohen's f value of 0.204. This suggests a small-medium effect size, which means that the type of drug and dosage level may still have had a meaningful impact on memory task performance. 
